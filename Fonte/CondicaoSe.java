	...
	public int Se(String[] linhas, int cont, int fim){
		...
		if(l.length > 3 && l.length < 7 && l[2].equals("%")){
			if(in.TestaString(l[1])){
				 ...
				 if(in.VerificaVariavel(l[1])) f = in.getValor(l[1]);
				 else{
					 System.out.println("VARIAVEL NAO LOCALIZADO NO SE!!!"); System.exit(0);
				 } 
			 }
			 if(in.TestaString(l[3])){
				g = Double.parseDouble(l[3]);
			 }else{
				if(in.VerificaVariavel(l[3]))  g = in.getValor(l[1]);
				else{
					System.out.println("VARIAVEL NAO LOCALIZADO NO SE!!!"); System.exit(0);
				}
			 }
			 if(in.TestaString(l[5])){
				 ...
				 if(in.VerificaVariavel(l[5])) e = in.getValor(l[5]);
				 else{
					 System.out.println("VARIAVEL NAO LOCALIZADO NO SE!!!"); System.exit(0);
				 }
			 }
			...
			if(in.TestaString(l[3])){
				...
				if(in.VerificaVariavel(l[3])) g = in.getValor(l[3]);
				else{
					System.out.println("VARIAVEL NAO LOCALIZADO NO SE!!!"); System.exit(0);
				}
			}	
